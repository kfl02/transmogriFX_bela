...
void set_efx_fbcomp_norm() {
    ...
    if ((rtprintclock >= 20000) && (doprint)) {
        rt_printf(
                "FB COMP PG1:  FB_COMP_THRS: %f\tFB_COMP_ATK: %f\tFB_COMP_RLS: %f\tFB_COMP_RATIO: %f\tFB_COMP_LEVEL: %f\tFB_COMP_MIX: %f\n",
...
void scan_inputs(BelaContext *context) {
    ...
    for (unsigned int n = 0; n < context->audioFrames; n++) {
        ...
        knobs[gRoundRobin].filtered_reading[0] = knobs[gRoundRobin].a * knobs[gRoundRobin].current_reading +
                                                 knobs[gRoundRobin].b * knobs[gRoundRobin].filtered_reading[0];
        knobs[gRoundRobin].filtered_reading[1] = knobs[gRoundRobin].c * knobs[gRoundRobin].filtered_reading[0] +
                                                 knobs[gRoundRobin].d * knobs[gRoundRobin].filtered_reading[1];
...
bool setup(BelaContext *context, void *userData) {
    ...
    gKnobToggleTimeout =
            (KNOB_SWITCH_TIMEOUT * ((unsigned int) context->audioSampleRate)) / (gNframes * ANALOG_CHANNELS);
