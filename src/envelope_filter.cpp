...
inline float soft_clip(float xn) {
    if (xn > 1.0f) {
        xn = 1.0f;
    } else if (xn < -1.0f) {
        xn = -1.0f;
...
inline float soft_gate(float xn_) {
    float xn = xn_ - 1.0f;
...
void envf_tick_n(env_filter *envf, float *x, float *e) {
    float ei;
    float a;
...
//settings
bool envf_toggle_bypass(env_filter *envf) {
    envf->bypass = !envf->bypass;

    return envf->bypass;
}

void envf_set_bypass(env_filter *envf, bool bp) {
    envf->bypass = bp;
}

void envf_set_q(env_filter *envf, float Q) {
    svf_set_q(envf->svf, Q);
}

void envf_set_drive(env_filter *envf, float drive_) {
    svf_set_drive(envf->svf, drive_);
}

void envf_set_mix(env_filter *envf, float mix_) {
    float mix = mix_;
...
void envf_set_mix_lpf(env_filter *envf, float mix_) {
    svf_set_mix_lpf(envf->svf, mix_);
}

void envf_set_mix_bpf(env_filter *envf, float mix_) {
    svf_set_mix_bpf(envf->svf, mix_);
}

void envf_set_mix_hpf(env_filter *envf, float mix_) {
    svf_set_mix_hpf(envf->svf, mix_);
}

void envf_set_normalize(env_filter *envf, bool n) {
    svf_set_normalize(envf->svf, n);
}

void envf_set_outclip(env_filter *envf, bool clip_output) {
    svf_set_outclip(envf->svf, clip_output);
}

void envf_set_atk(env_filter *envf, float t) {
    envf->atk_a = envf->ts / (envf->ts + t);
    envf->atk_b = 1.0f - envf->atk_a;
}

void envf_set_rls(env_filter *envf, float t) {
    envf->rls_a = envf->ts / (envf->ts + t);
    envf->rls_b = 1.0f - envf->rls_a;
}

void envf_set_sensitivity(env_filter *envf, float sns_) {
    float sns = sns_;
...
void envf_set_lfo_rate(env_filter *envf, float r_) {
    sample_hold_set_rate(envf->sh, r_);
}

void envf_set_depth(env_filter *envf, float d_) {
    float d = d_;
...
void envf_set_lfo_width(env_filter *envf, float w_) {
}

void envf_set_width(env_filter *envf, float w_) {
    float w = w_;
...
//  higher than -12 dB
void envf_set_gate(env_filter *envf, float thrs_) {
    float thrs = thrs_;
...
void envf_set_gate_knee(env_filter *envf, float knee_) {
    float knee = knee_;
...
void envf_set_mix_sh_modulator(env_filter *envf, float mix_) {
    float mix = mix_;
...
void envf_set_sample_hold_type(env_filter *envf, sh_mod_mode type) {
    sample_hold_set_type(envf->sh, type);
}

bool envf_set_adsr_active(env_filter *envf, bool act) {
    return sample_hold_set_active(envf->sh, act);
}

void envf_set_adsr_atk(env_filter *envf, float atk_) {
    float x = atk_;
...
void envf_set_adsr_dcy(env_filter *envf, float dcy_) {
    float x = dcy_;
...
void envf_set_adsr_stn(env_filter *envf, float stn_) {
    float x = stn_;
...
void envf_set_adsr_rls(env_filter *envf, float rls_) {
    float x = rls_;
